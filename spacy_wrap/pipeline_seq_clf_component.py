...
doc_extension_prediction = "seq_clf"
...
class ClassificationTransformer(TrainablePipe):
    ...
    def label_getter(self, doc) -> Optional[str]:
        ...
        if prob["prob"] is not None:
            return self.labels[int(prob["prob"].argmax())]
        return None
    ...
    def set_annotations(
        ...
        for doc, data in zip(docs, doc_data):
            ...
            setattr(doc._, self.doc_extension_prediction, self.label_getter(doc))
            setattr(
                doc._,
                f"{self.doc_extension_prediction}_prob",
                self.prob_getter(doc),
            )
    ...
    def initialize(
        ...
        *,
        ...
        # extract hf model
        hf_model = self.model.get_ref("transformer").model
        # extract hf model
